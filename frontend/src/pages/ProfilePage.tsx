...
function AvatarCard({ user }: { user: any }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 p-6 flex flex-col items-center text-center">
      ...
      <span className={`inline-block mt-3 text-xs font-medium px-3 py-1 rounded-full ${roleColors[user?.role] || 'bg-slate-100 text-slate-600'}`}>
        {user?.role}
      </span>
    </div>
  );
    ...
    <div className="bg-white rounded-xl border border-slate-100 p-6">
      <div className="flex items-center gap-2 mb-5">
        <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center">
          <i className="ti ti-clipboard-list text-blue-500 text-base" aria-hidden="true" />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-4">
          ...
          <InfoBlock label="Blood Type" value={data?.bloodType ? bloodTypeLabels[data.bloodType] : '—'} />
          <InfoBlock label="Insurance" value={data?.insuranceProviderName || '—'} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 p-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-4">
          <InfoBlock label="Specialization" value={data?.specialization || '—'} />
          <InfoBlock label="License Number" value={data?.licenseNumber || '—'} />
    ...
    <div className="bg-white rounded-xl border border-slate-100 p-6">
      <div className="flex items-center gap-2 mb-5">
        <div className="w-8 h-8 bg-purple-50 rounded-lg flex items-center justify-center">
          <i className="ti ti-shield text-purple-500 text-base" aria-hidden="true" />
        </div>
        <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wide">
          Admin Access
        </h3>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        {[
          ...
          <div key={item.label} className="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2.5">
            <i className={`ti ${item.icon} text-slate-400 text-sm`} aria-hidden="true" />
    ...
    <div className="bg-white rounded-xl border border-slate-100 p-6">
      <div className="flex items-center gap-2 mb-5">
        <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center">
          <i className="ti ti-lock text-slate-500 text-base" aria-hidden="true" />
        </div>
        <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wide">
          Security
        </h3>
