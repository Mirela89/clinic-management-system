      ...
      <div className="hidden lg:flex w-[44%] bg-slate-900 flex-col p-10 relative overflow-hidden flex-shrink-0">
        ...
        <div className="mb-auto pt-16">
          ...
          <p className="text-slate-500 text-sm leading-relaxed max-w-xs">
            Your medical information helps us provide better care and allows doctors to prepare for your appointments.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 pt-8 border-t border-white/5">
          {[
            { icon: 'ti-id', label: 'Personal details', desc: 'CNP and date of birth', s: 1 },
            { icon: 'ti-map-pin', label: 'Address & health', desc: 'Location and blood type', s: 2 },
          ].map(item => (
            <div key={item.s} className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${
                step >= item.s ? 'bg-blue-500' : 'bg-slate-800'
              }`}>
                <i className={`ti ${item.icon} text-sm ${step >= item.s ? 'text-white' : 'text-slate-500'}`} aria-hidden="true" />
              </div>
              <div>
                <p className={`text-sm font-medium ${step >= item.s ? 'text-white' : 'text-slate-500'}`}>
                  {item.label}
                </p>
                <p className="text-slate-600 text-xs">{item.desc}</p>
              </div>
              {step > item.s && (
                <i className="ti ti-circle-check text-blue-400 ml-auto text-base" aria-hidden="true" />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs">Complete your profile to continue</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-8">
            ...
            <div className={`flex-1 h-1 rounded-full transition-colors ${step === 2 ? 'bg-blue-500' : 'bg-slate-200'}`} />
              ...
              <>
                ...
                <div>
                  ...
                  <div className="relative">
                    <i className="ti ti-id absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-base pointer-events-none" aria-hidden="true" />
                    <input
                      name="cnp"
                      value={form.cnp}
                ...
                <div>
                  ...
                  <div className="relative">
                    <i className="ti ti-calendar absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-base pointer-events-none" aria-hidden="true" />
                    <input
                      name="dateOfBirth"
                      type="date"
                ...
                >
                  Continue
                  <i className="ti ti-arrow-right ml-2 text-sm" aria-hidden="true" />
                </button>
              </>
            )}
            ...
            {step === 2 && (
              <>
                ...
                <div>
                  ...
                  <div className="relative">
                    <i className="ti ti-map-pin absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-base pointer-events-none" aria-hidden="true" />
                    <input
                      name="address"
                      value={form.address}
                      onChange={handleChange}
                    ...
                    {BLOOD_TYPES.map(bt => (
                      <button
                        ...
                        onClick={() => setForm(prev => ({
                          ...prev,
                          bloodType: prev.bloodType === bt.value ? '' : bt.value
                        }))}
                ...
                <div className="flex gap-3 mt-1">
                  ...
                  >
                    <i className="ti ti-arrow-left mr-2 text-sm" aria-hidden="true" />
                    Back
                  </button>
                  <button
                    type="submit"
