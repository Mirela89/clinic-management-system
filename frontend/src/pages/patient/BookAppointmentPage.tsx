      ...
      {Array.from({ length: total }, (_, i) => (
        <div key={i} className="flex items-center gap-2">
          <div className="flex flex-col items-center gap-1">
            ...
            <span
              className={`text-xs hidden sm:block ${i + 1 === current ? "text-slate-700 font-medium" : "text-slate-400"}`}
            >
              {labels[i]}
            </span>
          </div>
          {i < total - 1 && (
            <div
              className={`h-px w-8 mb-4 ${i + 1 < current ? "bg-blue-500" : "bg-slate-200"}`}
            />
          )}
        </div>
      ))}
...
export default function BookAppointmentPage() {
  ...
  const filteredDoctors = doctors.filter(
    (d) => d.specialization === selectedSpeciality,
  );
  ...
  const { data: schedules = [], isLoading: loadingSchedule } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/api/doctor-schedules/doctor/${selectedDoctor!.userId}`,
      );
  ...
  const slots = selectedSchedule
    ? generateSlots(
        ...
        selectedSchedule.slotDurationMinutes,
      )
    : [];
    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Choose your specialist and find a time that works for you
        </p>
      </div>
              ...
              {specialities.map((s) => (
                ...
                >
                  <div className="w-8 h-8 bg-blue-50 group-hover:bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors">
                    <i
                      className="ti ti-stethoscope text-blue-500 text-sm"
                      aria-hidden="true"
                    />
                  </div>
              ...
              filteredDoctors.map((doctor) => {
                ...
                return (
                  ...
                  >
                    ...
                    <i
                      className="ti ti-chevron-right text-slate-300 ml-auto flex-shrink-0"
                      aria-hidden="true"
                    />
                  </button>
                );
          ...
          {loadingSchedule ? (
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="animate-pulse h-10 w-16 bg-slate-100 rounded-lg"
                />
              ))}
            </div>
          ) : schedules.length === 0 ? (
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {DAY_ORDER.filter((d) =>
                  schedules.some((s) => s.dayOfWeek === d),
                ).map((day) => (
                  <button
                    key={day}
                    onClick={() => {
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-6">
          ...
          >
            {submitting ? (
              ...
              <>
                <i
                  className="ti ti-calendar-plus text-base"
                  aria-hidden="true"
                />
                Confirm booking
              </>
