...
export default function PatientPrescriptionsPage() {
  ...
  const activeCount = prescriptions.filter(p => !isExpired(p.expiryDate)).length;
  const expiredCount = prescriptions.filter(p => isExpired(p.expiryDate)).length;
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">My Prescriptions</h1>
      ...
      {prescriptions.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-slate-200 p-12 text-center">
          <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <i className="ti ti-pill text-slate-300 text-2xl" aria-hidden="true" />
          </div>
          <h2 className="text-base font-semibold text-slate-900 mb-1">No prescriptions yet</h2>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="bg-white rounded-xl border border-slate-100 p-5">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Active prescription{activeCount !== 1 ? 's' : ''}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-100 p-5">
              <div className="w-9 h-9 bg-red-50 rounded-lg flex items-center justify-center mb-3">
                <i className="ti ti-clock-x text-red-400 text-base" aria-hidden="true" />
              </div>
              <p className="text-2xl font-semibold text-slate-900">{expiredCount}</p>
              <p className="text-xs text-slate-400 mt-0.5">Expired prescription{expiredCount !== 1 ? 's' : ''}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {sorted.map(p => {
              ...
              return (
                <div key={p.id} className={`bg-white rounded-xl border p-5 ${expired ? 'border-slate-100 opacity-75' : 'border-slate-100'}`}>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-slate-800 flex items-center justify-center text-white text-xs font-medium flex-shrink-0">
                        {p.doctorName.split(' ').map(n => n[0]).slice(0, 2).join('')}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-slate-900">Dr. {p.doctorName}</p>
                    ...
                    {expired ? (
                      <span className="flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full bg-red-50 text-red-500 border border-red-100">
                        <i className="ti ti-clock-x text-xs" aria-hidden="true" />
                  ...
                  <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-50">
                    <i className="ti ti-calendar text-slate-300 text-xs" aria-hidden="true" />
                    <span className="text-xs text-slate-400">Appointment #{p.appointmentId}</span>
                    <span className="text-slate-200 mx-1">·</span>
                    <i className="ti ti-file-text text-slate-300 text-xs" aria-hidden="true" />
                    <span className="text-xs text-slate-400">Consultation #{p.consultationId}</span>
                  </div>
                </div>
              );
            })}
